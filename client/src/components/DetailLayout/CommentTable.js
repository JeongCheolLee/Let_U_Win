...
function likeClickHandler(recordId, likeBefore) {
    axios
        .patch(`http://172.18.0.2:3001/comments/rating/like/${recordId}`, {
            like: likeBefore + 1,
        })
        .then((res) => {
            console.log(res);
...
function dislikeClickHandler(recordId, dislikeBefore) {
    axios
        .patch(`http://172.18.0.2:3001/comments/rating/dislike/${recordId}`, {
...
function CommentTable(props) {
    ...
    useEffect(() => {
        axios
            .get(`http://172.18.0.2:3001/comments/all/${myPick}/${enemyPick}`)
            .then((res) => {
